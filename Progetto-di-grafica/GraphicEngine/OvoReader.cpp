...
std::vector<Object*> OvoReader::readOVOfile(int argc, const char * argv)
{
	...
	while (true)
	{
		...
		{
			f << "ERROR: unable to read from file '" << argv[1] << "'" << endl;
			fclose(dat);
			delete[] data;
		}
		...
		case OvObject::Type::SKINNED:
		{
			...
			vector<unsigned int> facesArray;
			////////////////////////////////
			...
			for (unsigned int c = 0; c < vertices; c++)
			{
				...
				if (verbose)
					f << "      xyz  . . . :  " << vertex.x << ", " << vertex.y << ", " << vertex.z << endl;
				...
				if (verbose)
				{
					...
					f << "      normal . . :  " << normal.x << ", " << normal.y << ", " << normal.z << endl;
				...
				if (verbose)
				{
					...
					f << "      tangent  . :  " << tangent.x << ", " << tangent.y << ", " << tangent.z << ", sign: " << tangent.w << endl;
				}
				position += sizeof(unsigned int);
			}
		...
		default: //
			f << "UNKNOWN]" << endl;
			f << "ERROR: corrupted or bad data in file " << argv[1] << endl;
			fclose(dat);
			delete[] data;
